      ...
      <div class="lg:w-1/2 pt-8 lg:pb-8 flex flex-col md:h-[295px] lg:h-screen">
        ...
        >
          <li v-for="member in crew" :key="member.name">
            <button
              @click="select(member)"
              class="w-[10px] h-[10px] lg:w-[15px] lg:h-[15px] rounded-full hover:bg-white duration-500 ease-in-out"
              :class="{
                'bg-white': member.name === memberSelected.name,
                'bg-white/50 ': member.name !== memberSelected.name
              }"
            ></button>
          </li>
        </ul>
